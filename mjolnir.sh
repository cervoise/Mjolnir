...
#add support for hidden SSID (https://github.com/nttcomsecurity/PocketWifi/blob/master/Mjolnir/mjolnir.sh)
#add check for PSK length (between 8 and 64)
...
function mainGuess {
		...
		if [ ${#psk} -gt 7 ] && [ ${#psk} -lt 65 ] ; then
			...
			echo Skip "${psk}" (too short or too long)
		fi
        done
}
